...
function HelpSupportModalBody({
  ...
  const catalog = useMemo(() => supportApi.listSupportCatalog('technician'), []);
  const selectedCategory = useMemo(
    () => (categorySlug ? supportApi.getTechnicianSupportCategory(categorySlug) : undefined),
    [categorySlug],
  );
  ...
  const csatMut = useMutation({
    mutationFn: () => {
      ...
      return supportApi.submitSupportCsatAsTechnician(supabase, conversationId, {
        rating: csatRating,
        comment: csatComment,
      });
    },
  ...
  const createMut = useMutation({
    mutationFn: () => {
      ...
      return supportApi.createSupportConversationAsTechnician(supabase, {
  ...
  const sendMut = useMutation({
    mutationFn: (body: string) => {
      ...
      return supportApi.sendSupportMessageAsTechnician(supabase, {
  ...
  const markThreadRead = useCallback(
    async (convId: string) => {
      if (!supabase) return;
      await supportApi.markSupportConversationReadByTechnician(supabase, convId);
  ...
  const pickSubcategory = (subSlug: string, label: string) => {
    ...
    const sub = supportApi.getTechnicianSupportSubcategory(categorySlug!, subSlug);
    setLocalBubbles((prev) => [
      ...prev,
  ...
  const renderBubble = ({ item }: { item: SupportMessageRow | LocalBubble }) => {
    ...
    const isTechnician = msg.sender_role === "customer";
    ...
    return (
      <View
        style={[
          styles.bubbleRow,
          isTechnician ? styles.bubbleRowUser : styles.bubbleRowBot,
        ]}
      >
        <View
          style={[
            styles.bubble,
            isTechnician ? styles.bubbleUser : styles.bubbleAdmin,
          ]}
        >
          {!isTechnician ? <Text style={styles.senderTag}>OorjaMan Support</Text> : null}
          <Text
            style={[
              styles.bubbleText,
              isTechnician ? styles.bubbleTextUser : styles.bubbleTextBot,
            ]}
          >
            {msg.body}
          </Text>
