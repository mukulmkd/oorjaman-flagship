...
/**
 * Home-screen label + build-time Info.plist keys for “OorjaMan” / “OorjaMan (UAT)”.
 * Quotes are required in pbxproj when the name contains spaces.
 */
function withCustomerNativeBranding(config) {
  const displayName = config.name ?? "OorjaMan";
  ...
  config = withXcodeProject(config, (mod) => {
    ...
    getBuildConfigurationsForListId(
      ...
      if (
        ...
        bundleId.includes("oorjaman.customer")
      ) {
...
module.exports = withCustomerNativeBranding;
