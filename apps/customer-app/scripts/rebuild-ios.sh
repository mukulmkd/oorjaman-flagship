#!/usr/bin/env bash
# Full clean rebuild of the OorjaMan customer native iOS app (dev build on Simulator — not Expo Go).
set -euo pipefail
...
BUNDLE_ID="${EXPO_PUBLIC_IOS_BUNDLE_ID:-com.oorjaman.customer}"

echo "==> OorjaMan customer app: clean iOS rebuild"
echo "    Directory: $ROOT"
...
echo "==> Regenerate customer launcher icons and splash assets"
npm run brand:sync
